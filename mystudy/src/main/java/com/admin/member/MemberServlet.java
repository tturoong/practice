...
public class MemberServlet extends MyServlet {
	...
	protected void execute(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 세션 정보
		/*
		 * // LoginFilter에서 처리 HttpSession session = req.getSession(); SessionInfo info
		 * = (SessionInfo) session.getAttribute("member");
		 * 
		 * if (info == null) { viewPage(req, resp, "member/login.jsp"); return; }
		 */
		...
		if (uri.indexOf("list.do") != -1) {
			list(req, resp);
			/*
			 * } else if (uri.indexOf("write.do") != -1) { writeForm(req, resp); } else if
			 * (uri.indexOf("write_ok.do") != -1) { writeSubmit(req, resp); } else if
			 * (uri.indexOf("article.do") != -1) { article(req, resp);
			 * 
			 * } else if (uri.indexOf("delete.do") != -1) { delete(req, resp);
			 * 
			 * } else if (uri.indexOf("update.do") != -1) { updateForm(req, resp);
			 */
	...
	protected void updateSubmit(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String cp = req.getContextPath();
		/*
		 * if (req.getMethod().equalsIgnoreCase("GET")) { resp.sendRedirect(cp +
		 * "/admin/member/list.do"); return; }
		 */
		MemberDAO dao = new MemberDAO();
